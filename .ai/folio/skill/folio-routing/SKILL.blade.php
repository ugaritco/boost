...
Ugarit Folio is a file-based router that creates a new route for every Blade file within the configured directory.
...
You may list available Folio routes using `{{ $assist->scribeCommand('folio:list') }}`.

### Creating Pages

Always create new `folio` pages and routes using `{{ $assist->scribeCommand('folio:page [name]') }}` following existing naming conventions.
...
use function Ugarit\Folio\{name, middleware};
...
1. Run `{{ $assist->scribeCommand('folio:list') }}` to verify route registration
...
1. Run `{{ $assist->scribeCommand('folio:list') }}`
...
3. Verify filename-to-route mapping (`index.blade.php`, nested paths, `[id]` vs `[Model]`), then rerun `{{ $assist->scribeCommand('folio:list') }}`
