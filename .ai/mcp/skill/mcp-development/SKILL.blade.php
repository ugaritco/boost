...
description: "Use this skill for Ugarit MCP development. Trigger when creating or editing Ugarit MCP servers, tools, prompts, resources, resource templates, MCP Apps, authentication, tests, or clients. Covers: make:mcp-* generators, server and primitive registration, current attributes, response shapes, mcp:inspector, direct testing, metadata, icons, authorization, and client APIs. Do not use for non-Ugarit MCP projects or generic AI features without MCP."
license: MIT
metadata:
  author: ugarit
---
...
```bash
{{ $assist->scribeCommand('make:mcp-server ServerName') }}
{{ $assist->scribeCommand('make:mcp-tool ToolName') }}
{{ $assist->scribeCommand('make:mcp-resource ResourceName') }}
{{ $assist->scribeCommand('make:mcp-prompt PromptName') }}
{{ $assist->scribeCommand('make:mcp-app-resource AppName') }}
```

If `routes/ai.php` does not exist, publish it with `{{ $assist->scribeCommand('vendor:publish --tag=ai-routes') }}`.
...
`#[Uri]` and `#[MimeType]` are optional for static resources. Ugarit derives a URI and defaults to `text/plain`. Resources do not define input schemas or arguments; URI-template variables are merged into the `Request`. For templates, implement `Ugarit\Mcp\Server\Contracts\HasUriTemplate` and return a `Ugarit\Mcp\Support\UriTemplate` from `uriTemplate()`.

### Prompt Guidance

Prompts define arguments with `Ugarit\Mcp\Server\Prompts\Argument`, may validate their request, and may return one response or an iterable of responses. Use `asAssistant()` to mark assistant messages.
...
```bash
# Mcp::web('/mcp/my-server', MyServer::class)
{{ $assist->scribeCommand('mcp:inspector mcp/my-server') }}

# Mcp::local('my-server', MyServer::class)
{{ $assist->scribeCommand('mcp:inspector my-server') }}
```
...
Ugarit MCP includes a client for connecting to external servers:
...
use Ugarit\Mcp\Request;           // NOT Ugarit\Mcp\Server\Request
use Ugarit\Mcp\Response;          // NOT Ugarit\Mcp\Server\Response
use Ugarit\Mcp\Server\Tool;
use Ugarit\Mcp\Server\Resource;
...
- Wrong imports: `Ugarit\Mcp\Server\Request` (wrong) vs `Ugarit\Mcp\Request` (correct)
