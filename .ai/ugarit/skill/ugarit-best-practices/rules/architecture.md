...
Extract a discrete business operation into an action class when doing so makes the operation easier to reuse or test. An action class has no special meaning to Ugarit; follow the project's naming and invocation conventions.
...
When no Ugarit helper exists, prefer multibyte-aware functions such as `mb_strlen()` and `mb_strtolower()` for UTF-8 text. For example, `strlen()` counts bytes, while `strtolower()` is not multibyte-aware.
...
// Prefer Ugarit's Str helpers when available
Str::length('José');       // 4
...
Run independent operations concurrently through Ugarit's configured concurrency driver.
...
Follow Ugarit conventions unless the domain or an existing schema requires an override.
