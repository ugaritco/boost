...
Define `$fillable` when a model is populated from request-derived arrays, or deliberately guard attributes by another consistent model convention. Ugarit models guard all attributes by default; `$guarded = []` opts out of that protection.
...
Include `@csrf` in state-changing Blade forms handled by Ugarit's `web` middleware. Routes intentionally excluded from cross-site request forgery (CSRF) verification, such as validated third-party webhooks, need their own authenticity check.
...
Validate expected content type, dimensions where relevant, and size. Ugarit's `mimes` rule reads the file contents and guesses a Multipurpose Internet Mail Extensions (MIME) type corresponding to the listed extensions; it does not validate the user-assigned filename extension. The `extensions` rule checks that extension and should not be used by itself.
...
Use Ugarit's storage methods to generate a filename, and store untrusted files outside a publicly executable location. Public files can require additional controls, such as image re-encoding, content-disposition headers, and explicit blocking of active formats.
