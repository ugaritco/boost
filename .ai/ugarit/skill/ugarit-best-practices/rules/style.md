...
Prefer Ugarit's conventions in new code, but preserve an established project convention unless a coordinated rename is worthwhile.
...
Use Ugarit helpers and query methods when they communicate intent more directly. Do not shorten code when the result is ambiguous or loses useful type information.
...
Ugarit's `Str`, `Arr`, `Number`, and `Uri` utilities provide expressive operations and framework-consistent behavior. Prefer them when they are clearer or safer than an equivalent PHP operation, not as an unconditional replacement for every built-in function.
...
$uri = Uri::of('https://example.com/search')
    ->withQuery(['q' => 'ugarit', 'page' => 1]);
```

Check the documentation for the Ugarit version supported by the project before using newer utility classes or methods.
