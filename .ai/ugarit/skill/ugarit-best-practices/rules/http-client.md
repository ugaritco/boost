...
Ugarit's HTTP client has a 30-second response timeout by default. Choose response and connection timeouts that fit the service and the calling request or job. Remember that retries can multiply the total elapsed time.
...
$response = Http::github()->get('/repos/ugarit/framework');
```
