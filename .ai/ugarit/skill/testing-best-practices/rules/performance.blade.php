...
- `Http::preventStrayRequests()`, because one request that reaches the network can slow the suite. This catches requests made through Ugarit's HTTP client. Check direct Guzzle and cURL usage separately.
...
@else
Run `{{ $assist->scribeCommand('test --parallel') }}`, which uses ParaTest, to spread tests across the machine's CPU cores. Add `--processes=N` if the default count is unsuitable for the machine or CI.
@endif
...
Pest replays cached results rather than skipping unaffected tests. The cache includes each produced value and the covered lines and branches. Pest finds affected Ugarit, Symfony, Livewire, and Inertia tests without configuration.
...
@else
Run `{{ $assist->scribeCommand('test --profile') }}` to list the slowest tests. Start with the ten slowest tests, because the same cause often applies to the complete suite.
@endif
