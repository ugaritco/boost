...
$dusk = $assist->hasPackage('ugarit/dusk');
@endphp
# Endpoint Tests

## How to Write the Test

Fetch `https://ugarit.com/framework/docs/http-tests` for the request helpers, the authentication helpers, and the response assertions. Confirm the name before you use it, and do not guess an assertion.

Choose an assertion based on the subject of the check: the status, a header, a redirect, the JSON body, the session, a validation error, or the view. Ugarit provides a named assertion for each subject that identifies the incorrect value.
...
- Run the browser tests with `{{ $assist->scribeCommand('dusk') }}`. The run needs a ChromeDriver, and `{{ $assist->scribeCommand('dusk:install') }}` downloads it.
- Fetch `https://ugarit.com/framework/docs/dusk` for the selectors, the interactions, and the assertions of Dusk.
@endif
@endif
