...
1. Search Ugarit's assertions for framework subjects such as responses, the database, sessions, models, queues, events, mail, and notifications.
...
| An HTTP status, JSON, a session, or Inertia | a Ugarit response assertion |
| The state in the database | a Ugarit database assertion |
| The existence of a model | `assertModelExists($model)` rather than `assertDatabaseHas('users', ['id' => $user->id])` |

Use a PHPUnit assertion only if no Pest expectation and no Ugarit assertion exists for the subject.
@else
| Subject | Assertion to use |
| --- | --- |
...
Use a named response assertion, such as `assertNotFound()`, rather than `assertStatus(404)`. A failure then identifies the broken contract. Ugarit provides named assertions for commonly tested status codes.
