...
@else
PHPUnit and Ugarit provide features for most testing needs. Find an existing feature before implementing the behavior by hand.

- Give `search-docs` the capability you need rather than the name of a method you remember. It returns Ugarit testing documentation for the installed version.
...
@if($pest)
expect(User::where('email', 'taylor@ugarit.com')->exists())->toBeTrue();
@else
$this->assertTrue(User::where('email', 'taylor@ugarit.com')->exists());
@endif

// Use this... the failure names the table and the attributes that it did not find...
$this->assertDatabaseHas('users', ['email' => 'taylor@ugarit.com']);
```
