...
This skill provides rules for designing Ugarit tests. Each rule file explains what to do and why. Use `search-docs` for {{ $pest ? 'Ugarit and Pest API syntax' : 'Ugarit API syntax' }}.@if(! $pest) Fetch {{ $phpunitDocs }} for PHPUnit API syntax.@endif
...
@elseif($assist->hasPackage('ugarit/dusk'))
- Write a Dusk test only for behavior in JavaScript that a feature test cannot reach. Put a Dusk test in `tests/Browser`.
@else
- Write a feature test for every behavior reachable through a request. Real-browser tests require {{ $pest ? '`pestphp/pest-plugin-browser`' : '`ugarit/dusk`' }} and a browser download, neither of which this project installs. Mention the package only if the user asks for a real-browser test.
@endif
@if($pest)
