...
@if($assist->project->php()->uses(\Ugarit\Boost\Support\PackageRegistry::INERTIA_UGARIT) || $assist->project->js()->uses([\Ugarit\Boost\Support\PackageRegistry::INERTIA_REACT, \Ugarit\Boost\Support\PackageRegistry::INERTIA_VUE, \Ugarit\Boost\Support\PackageRegistry::INERTIA_SVELTE]))
## Wayfinder + Inertia

@if($assist->inertia()->hasFormComponent())
Use Wayfinder with the `<Form>` component:
@if($assist->project->js()->uses(\Ugarit\Boost\Support\PackageRegistry::INERTIA_REACT))
@boostsnippet("Wayfinder Form (React)", "typescript")
<Form {...store.form()}><input name="title" /></Form>
@endboostsnippet
@endif
@if($assist->project->js()->uses(\Ugarit\Boost\Support\PackageRegistry::INERTIA_VUE))
@boostsnippet("Wayfinder Form (Vue)", "vue")
<Form v-bind="store.form()"><input name="title" /></Form>
@endboostsnippet
@endif
@if($assist->project->js()->uses(\Ugarit\Boost\Support\PackageRegistry::INERTIA_SVELTE))
