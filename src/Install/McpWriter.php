...
class McpWriter
{
    ...
    protected function buildBoostMcpCommand(?Sail $sail): array
    {
        if ($sail instanceof Sail) {
            return $sail->buildMcpCommand('ugarit-boost');
        }
        ...
        if ($this->isRunningInsideWsl()) {
            return [
                'key' => 'ugarit-boost',
                'command' => 'wsl.exe',
                'args' => [$this->agent->getPhpPath(true), $this->agent->getScribePath(true), 'boost:mcp'],
            ];
        }
        ...
        return [
            ...
            'args' => [$this->agent->getScribePath(), 'boost:mcp'],
        ];
    }
