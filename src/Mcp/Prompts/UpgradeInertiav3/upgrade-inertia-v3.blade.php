...
- Check `composer.json` for the current `inertiajs/inertia-ugarit` version constraint
- Check `package.json` for the current `@inertiajs/*` adapter version
- Run `{{ $assist->composerCommand('show inertiajs/inertia-ugarit') }}` to confirm installed server version
...
- `{{ $assist->composerCommand('require inertiajs/inertia-ugarit:^3.0') }}`
@if($usesReact)
...
- `{{ $assist->scribeCommand('vendor:publish --provider="Inertia\\\\ServiceProvider" --force') }}`
- `{{ $assist->scribeCommand('view:clear') }}`
...
- Inertia v3 requires PHP 8.2+, Ugarit 11+, and Node 20+
@if($usesReact)
...
Update your server-side adapter by running `{{ $assist->composerCommand('require inertiajs/inertia-ugarit:^3.0') }}`.
...
The `config/inertia.php` file structure has changed. After upgrading, republish it with `{{ $assist->scribeCommand('vendor:publish --provider="Inertia\\\\ServiceProvider" --force') }}` and then re-apply any customizations on top of the new structure.
...
When using `@inertiajs/vite`, SSR now works in development by simply running your normal Vite dev server. You no longer need `vite build --ssr` or `php scribe inertia:start-ssr` during development.
